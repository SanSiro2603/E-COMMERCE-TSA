...
@section('content')
    <div class="bg-white dark:bg-zinc-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-zinc-700">
        <h2 class="text-lg font-bold text-gray-900 dark:text-white mb-4">Pengaturan Akses Customer</h2>
        ...
        <form action="{{ route('admin.settings.update') }}" method="POST">
            ...
            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-zinc-700/50 rounded-lg">
                <div>
                    <h3 class="font-semibold text-gray-900 dark:text-white">Login Customer</h3>
                    <p class="text-sm text-gray-500 dark:text-zinc-400">
                        Jika dimatikan, tombol login, register, Google OAuth, dan reset password customer disembunyikan. Admin tetap dapat login.
                    </p>
                </div>

                {{-- TOGGLE SWITCH --}}
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="customer_login_enabled" value="1" class="sr-only peer"
                        {{ $customerLoginEnabled ? 'checked' : '' }}>
                    <div
