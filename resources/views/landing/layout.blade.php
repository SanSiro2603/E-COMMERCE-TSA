...
<body class="bg-tsa-soft font-sans text-slate-900 antialiased">
    @php
    ...
    <header class="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur-md"
            x-data="{ mobileOpen: false, infoOpen: false }">
        <div class="mx-auto flex h-16 w-[94%] max-w-[1240px] items-center justify-between gap-6">
            ...
            <a href="{{ route('landing') }}" class="flex shrink-0 items-center gap-3" aria-label="Tunas Sejahtera Adhi Perkasa">
                <img src="{{ asset('images/logo header.png') }}"
                     alt="TSA logo"
                     class="h-10 w-10 rounded-full object-cover ring-2 ring-tsa-green/20">
            ...
            <nav class="hidden items-center gap-7 lg:flex" aria-label="Main Navigation">
                ...
                <a href="{{ route('landing.catalog') }}"
                   class="relative pb-0.5 text-[14px] font-bold transition-colors duration-150
                          {{ request()->routeIs('landing.catalog') ? 'text-tsa-greenDark' : 'text-slate-800 hover:text-tsa-greenDark' }}"
                   x-data @mouseenter="$el.querySelector('span').style.width='100%'" @mouseleave="$el.querySelector('span').style.width='{{ request()->routeIs('landing.catalog') ? '100%' : '0%' }}'">
                    Catalog
                    <span class="absolute bottom-0 left-0 h-0.5 rounded-full bg-tsa-green transition-all duration-200"
                          style="width: {{ request()->routeIs('landing.catalog') ? '100%' : '0%' }}"></span>
                </a>
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="hidden items-center gap-1 rounded-lg border border-slate-200 p-1 lg:flex">
                    <a href="{{ route('lang.switch', 'id') }}"
                       class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[12px] font-semibold transition-colors duration-150
                              {{ $locale === 'id' ? 'bg-tsa-green text-white' : 'text-slate-500 hover:text-slate-800' }}">
                    ...
                    <a href="{{ route('lang.switch', 'en') }}"
                       class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[12px] font-semibold transition-colors duration-150
                              {{ $locale === 'en' ? 'bg-tsa-green text-white' : 'text-slate-500 hover:text-slate-800' }}">
                        <span class="inline-flex h-3.5 w-3.5 overflow-hidden rounded-full" aria-hidden="true">
        ...
        <div x-show="mobileOpen"
            ...
            <nav class="space-y-0.5">
                ...
                <div class="flex items-center gap-2 pt-2">
                    <a href="{{ route('lang.switch', 'id') }}"
                       class="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border py-2 text-sm font-semibold transition-colors duration-150
                              {{ $locale === 'id' ? 'border-tsa-green bg-tsa-green/10 text-tsa-greenDark' : 'border-slate-200 text-slate-600 hover:bg-slate-50' }}">
                    ...
                    <a href="{{ route('lang.switch', 'en') }}"
                       class="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border py-2 text-sm font-semibold transition-colors duration-150
                              {{ $locale === 'en' ? 'border-tsa-green bg-tsa-green/10 text-tsa-greenDark' : 'border-slate-200 text-slate-600 hover:bg-slate-50' }}">
                        <span class="inline-flex h-4 w-4 overflow-hidden rounded-full" aria-hidden="true">
